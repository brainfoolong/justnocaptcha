...
func (pc *JustNoCaptcha) CreateChallenge(puzzles int, difficulty int) (string, error) {
	if puzzles <= 0 {
		puzzles = 50
	}
	if len(pc.ChallengeSalt) <= 0 {
		return "", errors.New("JustNoCaptcha.challengeSalt required, should be a random value not exposed to solver clients")
	}
...
func (pc *JustNoCaptcha) VerifySolution(challengeData, solution string) (bool, error) {
	...
	if pc.VerifiedSolutionsFolder == "" {
		return false, errors.New("cannot find VerifiedSolutionsFolder for JustNoCaptcha")
	}
...
func (pc *JustNoCaptcha) SolveChallenge(challengeData string) (string, error) {
...
func (pc *JustNoCaptcha) Hash(data string) string {
...
func (pc *JustNoCaptcha) parseChallengeData(challengeData string, validateChallenge bool) (ChallengeResult, error) {
	...
	if validateChallenge && pc.ChallengeSalt == "" {
		return ChallengeResult{}, errors.New("JustNoCaptcha.challengeSalt required, should be a random value not exposed to solver clients")
	}
