// JustNoCaptcha v0.1.0 @ https://github.com/brainfoolong/justnocaptcha
export default class JustNoCaptcha {
    // browser-strip-start
    // @ts-ignore
    static verifiedSolutionsFolder = process?.env?.JUSTNOCAPTCHA_VERIFIED_SOLUTIONS_FOLDER;
    // browser-strip-end
    ...
    static createChallenge(puzzles = 50, difficulty = 4) {
        if (!JustNoCaptcha.challengeSalt) {
            throw new Error('JustNoCaptcha.challengeSalt required, should be a random value not exposed to solver clients');
        }
        let randomHash;
        ...
        return challenge + JustNoCaptcha.hash(challenge + JustNoCaptcha.challengeSalt);
    }
    /**
    ...
    static verifySolution(challengeData, solution) {
        const challengeMeta = JustNoCaptcha.parseChallengeData(challengeData, true);
        ...
        const hashFile = JustNoCaptcha.verifiedSolutionsFolder + '/' + JustNoCaptcha.hash(challengeData) + '.pow';
        ...
        for (let i = 0; i < challengeMeta.numberPuzzles; i++) {
            ...
            if (JustNoCaptcha.hashInt(challengeMeta.puzzlesString.substring(i * 32, i * 32 + 32) + iteration) <= challengeMeta.threshold) {
                continue;
            }
            return false;
        }
        // browser-strip-start
        if (!JustNoCaptcha.verifiedSolutionsFolder || !fs.existsSync(JustNoCaptcha.verifiedSolutionsFolder) || !fs.lstatSync(JustNoCaptcha.verifiedSolutionsFolder).isDirectory()) {
            throw new Error('Cannot write to verifiedSolutionsFolder for JustNoCaptcha');
        }
        // delete files older than 5 minutes
        const timeThreshold = new Date().getTime() - (300 * 1000);
        for (const file of fs.readdirSync(JustNoCaptcha.verifiedSolutionsFolder)) {
            const path = JustNoCaptcha.verifiedSolutionsFolder + '/' + file;
    ...
    static async solveChallenge(challengeData, progressHandler = null) {
        const challengeMeta = JustNoCaptcha.parseChallengeData(challengeData, false);
        const totalWorkers = challengeMeta.numberPuzzles;
        if (typeof window !== 'undefined' && typeof Worker === 'function') {
            let workerContentsBase = JustNoCaptcha.toString() + ';\n';
            const promises = [];
            let doneWorkers = 0;
            for (let i = 0; i < totalWorkers; i++) {
                let workerContents = workerContentsBase;
                const challenge = challengeMeta.puzzlesString.substring(i * 32, i * 32 + 32);
                workerContents += '(async()=>{self.postMessage(JustNoCaptcha.solverWorker(' + JSON.stringify(challenge) + ', ' + JSON.stringify(challengeMeta.difficulty) + '))})()';
            ...
            let solutions = '';
            for (let i = 0; i < totalWorkers; i++) {
                solutions += JustNoCaptcha.solverWorker(challengeMeta.puzzlesString.substring(i * 32, i * 32 + 32), challengeMeta.difficulty);
                if (progressHandler) {
     ...
     */
    static hash(data) {
        data = JustNoCaptcha.toUint8Array(data);
        let h1 = 0x811c9dc5;
        let h2 = 0x8b8d2a97;
        let h3 = 0xc9dc5118;
        let h4 = 0x7b9d8b8d;
        ...
        return JustNoCaptcha.fmix(h1) + JustNoCaptcha.fmix(h2) + JustNoCaptcha.fmix(h3) + JustNoCaptcha.fmix(h4);
    }
    /**
     * Generate a compute intensive but non cryptographic purpose fixed length hash
     * @param {BufferSource|Uint8Array|string} data
    ...
    static hashInt(data) {
        ...
        return JustNoCaptcha.fmix(h1, false);
    }
    /**
    ...
    static solverWorker(challenge, difficulty = 4) {
        ...
        while (iteration <= maxIterations) {
            if (JustNoCaptcha.hashInt(challenge + iteration) <= threshold) {
    ...
    static toUint8Array(data) {
        if (!JustNoCaptcha.encoder) {
            const encoder = new TextEncoder();
            JustNoCaptcha.encoder = (data) => {
        ...
        return JustNoCaptcha.encoder(data);
    }
    static parseChallengeData(challengeData, validateChallenge) {
        ...
        if (validateChallenge && !JustNoCaptcha.challengeSalt) {
            throw new Error('JustNoCaptcha.challengeSalt required, should be a random value not exposed to solver clients');
        }
        ...
        if (validateChallenge) {
            const challengeHashCalculated = JustNoCaptcha.hash(challengeData.substring(0, challengeData.length - 32) + JustNoCaptcha.challengeSalt);
